...
export default function MonthRibbon({
  ...
  return (
    <motion.div
      ...
      className={`mb-3 w-full overflow-hidden rounded-xl border border-zinc-200 bg-gradient-to-r ${accentClass}`}
    >
      <div className="flex flex-wrap items-center gap-3 px-3 py-2">
        <span className={`text-sm font-semibold ${textClass}`}>{label}</span>
        <span className="h-4 w-px bg-zinc-200/70" />
...
function Badge({ children }: { children: React.ReactNode }) {
  return (
    <span className="rounded-md border border-zinc-200 bg-white/60 px-2 py-0.5 text-xs text-zinc-700 backdrop-blur">
      {children}
    </span>
  );
}
